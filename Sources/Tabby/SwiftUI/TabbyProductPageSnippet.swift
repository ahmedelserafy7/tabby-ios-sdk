...
public struct TabbyProductPageSnippet: View {
  ...
  public var body: some View {
    ...
    let amountInString = String(amount/4) ?? ""
    let textNode2 = kind == .common ? String(format: "snippetAmount".localized, "\((amountInString).withFormattedAmount)", "\(currency.localized(l: withCurrencyInArabic && isRTL ? .ar : nil))") : String(format: "snippetAmountEG".localized, "\((amountInString).withFormattedAmount)", "\(currency.localized(l: withCurrencyInArabic && isRTL ? .ar : nil))")
