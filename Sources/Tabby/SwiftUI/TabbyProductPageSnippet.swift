...
public struct TabbyProductPageSnippet: View {
  ...
  public var body: some View {
    ...
    let textNode2 = kind == .common ? String(format: "snippetAmount".localized, "\((amount ?? 0/4).withFormattedAmount)", "\(currency.localized(l: withCurrencyInArabic && isRTL ? .ar : nil))") : String(format: "snippetAmountEG".localized, "\((amount/4).withFormattedAmount)", "\(currency.localized(l: withCurrencyInArabic && isRTL ? .ar : nil))")
